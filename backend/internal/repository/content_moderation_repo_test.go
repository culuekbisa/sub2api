...
func TestContentModerationRepositoryEngineMetaInsert(t *testing.T) {
	for _, meta := range []*service.ContentModerationEngineMeta{nil, {Engine: "typesafe", Model: "jev-fixed", RulesVersion: "rules-v1", SkippedImages: 1}} {
		...
		// The merged schema is the union of both sides: the fork's four extra
		// columns (moderation_endpoint_id, moderation_endpoint_name,
		// input_content, input_content_truncated) plus upstream's engine_meta,
		// which stays the trailing argument.
		...
		if meta == nil {
			...
			args[29] = `{"engine":"typesafe","model":"jev-fixed","rules_version":"rules-v1","skipped_images":1}`
		}
...
func TestContentModerationRepositoryEngineMetaRead(t *testing.T) {
	for _, meta := range []any{nil, `{"engine":"typesafe","model":"jev-fixed","rules_version":"rules-v1","skipped_images":1}`} {
		...
		// Column order mirrors the merged SELECT/Scan pair: the fork's endpoint and
		// input_content columns, then upstream's engine_meta last.
		columns := []string{"id", "request_id", "user_id", "user_email", "api_key_id", "api_key_name", "group_id", "group_name", "moderation_endpoint_id", "moderation_endpoint_name", "endpoint", "provider", "model", "mode", "action", "flagged", "highest_category", "highest_score", "category_scores", "threshold_snapshot", "input_excerpt", "input_content", "input_content_truncated", "upstream_latency_ms", "error", "violation_count", "auto_banned", "email_sent", "status", "queue_delay_ms", "matched_keyword", "created_at", "engine_meta"}
		mock.ExpectQuery("SELECT[\\s\\S]*l.engine_meta").WillReturnRows(sqlmock.NewRows(columns).AddRow(1, "req", nil, "", nil, "", nil, "", "default", "OpenAI Moderation", "/v1/responses", "business", "gpt-model", "observe", "allow", false, "sexual", 0.1, `{"sexual":0.1}`, `{"sexual":0.8}`, "sample", "full sample", false, 10, "", 0, false, false, "active", nil, "", time.Now(), meta))
