...
func openPromptAuditIntegrationDB(t *testing.T) *sql.DB {
	...
	`)
	require.NoError(t, err)
	for _, name := range []string{"181_prompt_audit.sql", "182_prompt_audit_full_prompt.sql", "236_prompt_audit_observability.sql", "237_prompt_audit_client_ip_index_notx.sql"} {
...
func TestPromptAuditObservabilityMigrationBackfillsHistoricalTruncation(t *testing.T) {
	...
	migration, err := os.ReadFile(filepath.Join("..", "..", "migrations", "236_prompt_audit_observability.sql"))
	require.NoError(t, err)
	_, err = db.ExecContext(ctx, string(migration))
	require.NoError(t, err)
