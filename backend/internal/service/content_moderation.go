...
// checkSync runs one synchronous audit pass. sideEffects is optional so that
// callers which only need the decision (and upstream tests) keep working; it
// defaults to true, meaning hits apply their normal account side effects.
func (s *ContentModerationService) checkSync(ctx context.Context, input ContentModerationCheckInput, cfg *ContentModerationConfig, content ContentModerationInput, hashText string, queueDelay *int, allowBlock bool, sideEffects ...bool) *ContentModerationDecision {
	applySideEffects := true
	if len(sideEffects) > 0 {
		applySideEffects = sideEffects[0]
	}
	allow := &ContentModerationDecision{Allowed: true, Action: ContentModerationActionAllow}
