export default {
    ...
    riskControl: {
      ...
      textApiMode: '文本审核 API 策略',
      textApiModes: {
        observe: '影子观察',
        off: '关闭文本 API',
      },
      policyHelp: {
        open: '查看全局模式与文本 API 策略矩阵',
        title: '内容审计策略组合',
        summary: '全局模式决定内容审计同步/异步运行方式及最高拦截权限；文本 API 策略只细化文本 Moderation API 的调用与决策，图片、关键词和哈希仍遵循各自控制项。',
        current: '当前生效行为：',
        globalMode: '全局模式',
      ...
      endpointPool: {
        title: '审核端点池',
        hint: '按列表顺序调用端点；平台失败会进入冷却并立即切换。冷却结束不主动探活，下一次真实审核请求会验证恢复情况。',
        add: '添加端点',
        unnamed: '未命名端点',
        defaultName: '审核端点 {index}',
        name: '端点名称',
        apiKeys: 'API Key',
        keysKeep: '已保存 {count} 个 Key，留空保持不变',
        keysRequired: '至少输入一个 API Key',
        keysHint: '输入非空内容会替换该端点的 Key；完整密钥不会返回页面。',
        cooldown: '冷却时间（秒）',
